...
type
  TPathGroup = record
    Color: TColor;
    Alpha: Byte;
    PathData: TStringBuilder;
  end;
var
  LGroups: TList<TPathGroup>;
  LRect: TColorRect;
  LIndex, LGroupIndex: Integer;
  LPathGroup: TPathGroup;
  LFound: Boolean;
  LColorStr: string;
  LOpacity: string;
...
begin
  LGroups := TList<TPathGroup>.Create;
  try
    ...
    begin
      ...
      begin
        LPathGroup := LGroups[LIndex];

        if (LPathGroup.Color = LRect.Color) and (LPathGroup.Alpha = LRect.Alpha) then
        begin
          LFound := True;
          LGroupIndex := LIndex;
      ...
      if not LFound then
      begin
        LPathGroup.Color := LRect.Color;
        LPathGroup.Alpha := LRect.Alpha;
        LPathGroup.PathData := TStringBuilder.Create;

        LGroups.Add(LPathGroup);

        LGroupIndex := LGroups.Count - 1;
      end;

      LGroups[LGroupIndex].PathData.AppendFormat('M%d %dh%dv%dh-%dZ', [LRect.X, LRect.Y, LRect.Width, LRect.Height, LRect.Width]);
    end;
    ...
    begin
      ...
      if LPathGroup.Color = 0 then
        LColorStr := ''
      else
        LColorStr := 'fill="#' + ColorToHex(LPathGroup.Color) + '" ';

      if LPathGroup.Alpha = 255 then
        LOpacity := ''
      else
        LOpacity := 'fill-opacity="' + FormatFloat('.###', LPathGroup.Alpha / 255, TFormatSettings.Invariant) + '" ';

      AOutput.Add(Format('  <path %s%sd="%s"/>', [LColorStr, LOpacity, LPathGroup.PathData.ToString.Trim]));

      LPathGroup.PathData.Free;
    end;
  finally
    LGroups.Free;
  end;
